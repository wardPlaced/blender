...
void BL_Converter::MergeAsyncLoads()
{
	...
	for (KX_LibLoadStatus *libload : m_mergequeue) {
		...
		for (BL_SceneConverter& converter : libload->GetSceneConverters()) {
			KX_Scene *scene = converter.GetScene();
			mergeScene->Merge(scene);
			// Finalize material and mesh conversion.
			converter.Finalize(mergeScene);
			delete scene;
		}
...
KX_LibLoadStatus *BL_Converter::LinkBlendFile(BlendHandle *blendlib, const char *path, char *group, KX_Scene *scene_merge, char **err_str, short options)
{
	...
	if (idcode == ID_ME || idcode == ID_AC) {
		...
		// Finalize material and mesh conversion.
		sceneConverter.Finalize(scene_merge);

		// Merge ressources (mesh or actions) to the targeted scene ressource collection.
		BL_ResourceCollection ressources(sceneConverter);
		scene_merge->GetResources().Merge(ressources);
	}
		...
		if (options & LIB_LOAD_LOAD_ACTIONS) {
			...
			for (ID *action = (ID *)main_newlib->action.first; action; action = (ID *)action->next) {
				if (options & LIB_LOAD_VERBOSE) {
					CM_Debug("action name: " << action->name + 2);
				}
				sceneConverter.RegisterAction((bAction *)action);
			}
...
KX_Mesh *BL_Converter::ConvertMeshSpecial(KX_Scene *kx_scene, Main *maggie, const std::string& name)
{
	...
	// Finalize material and mesh conversion.
	sceneConverter.Finalize(kx_scene);
	BL_ResourceCollection ressources(sceneConverter);
